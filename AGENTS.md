...
- **2026-08-28 Add Expand Personnel Mode in Org Chart**:
  ...
  - **Implementation**: Utilized existing `_orgMemberPrefetchState.byCode` memory cache for zero-latency personnel injection in `paintOrg()`. Added `deduplicatePersonnel_` to deduplicate personnel by email/name while prioritizing titles containing "長" (e.g. 組長/站長) when multiple assignments exist for the same person in a node. Enhanced `Components.orgChartNode_` in `js/components.html` and added compact vertical styling in `styles.html` to prevent horizontal width explosion. Fixed PNG export (`_showOrgExportModal`) by passing the personnel-injected tree, and hardened off-screen DOM rendering in `exportOrgChartPng` for full personnel capsule export compatibility.
